...
export default function AdminSecurityLogPage(){
  ...
  async function load(){ setLoading(true); const l = await builder.fetchSecurityLogs({ userId: userId||undefined, action: action||undefined }); setLogs(l); setLoading(false); }
    ...
    <Layout>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <div className="text-sm text-muted">Audit interne et tentatives d'accès</div>
        </div>
      </div>

      <div className="card p-4 mb-6">
        <div className="flex gap-2">
          <input className="input" placeholder="Utilisateur ID" value={userId} onChange={(e)=>setUserId(e.target.value)} />
          ...
          <button className="btn-ghost" onClick={async ()=>{ setUserId(''); setAction(''); await load(); }}>Réinitialiser</button>
        </div>
      </div>
