...
export default function AuditsPage() {
  ...
  const statusBadge = (s: string | undefined) => {
    ...
    switch (status) {
      case "completed":
      case "done":
        return <span className="px-2 py-1 rounded text-white text-xs" style={{ background: "#16a34a" }}>✓ {s}</span>;
      case "in_progress":
      case "in-progress":
      case "ongoing":
        return <span className="px-2 py-1 rounded text-white text-xs" style={{ background: "#f59e0b" }}>● {s}</span>;
      case "planned":
      case "draft":
        return <span className="px-2 py-1 rounded text-black text-xs" style={{ background: "#93c5fd" }}>{s}</span>;
      case "cancelled":
      case "canceled":
        return <span className="px-2 py-1 rounded text-white text-xs" style={{ background: "#ef4444" }}>{s}</span>;
      default:
        return <span className="px-2 py-1 rounded text-sm border border-border text-sm">{s || "—"}</span>;
    }
  };
      ...
      <section>
        {loading ? (
          ...
          <div className="card text-center" style={{ color: "var(--text)" }}>
            ...
            <div className="text-sm mt-2 mb-4">Il n'y a pas d'audits disponibles pour le moment.</div>
            <div>
              <button onClick={() => navigate('/audit/new')} className="btn-primary btn-sm">Créer un audit</button>
            </div>
          </div>
        ) : (
