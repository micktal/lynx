...
export default function Index() {
  ...
  const perSiteCounts = useMemo(async () => {
    ...
    return data.perSite.reduce(
      (
        acc: Record<
          string,
          { buildings: number; spaces: number; equipments: number }
        >,
        cur,
      ) => {
        acc[cur.siteId] = {
  ...
  const [countsMap, setCountsMap] = useState<
    Record<string, { buildings: number; spaces: number; equipments: number }>
  >({});
  ...
  const handleSave = async (payload: Partial<Site>) => {
    if (editing) {
      ...
      setSites((prev) =>
        prev.map((p) => (p.id === editing.id && updated ? updated : p)),
      );
    } else {
  ...
  const exportCsv = () => {
    const rows = visible.map((s) =>
      [s.id, s.name, s.city, s.country, s.address]
        .map((c) => `"${(c || "").replace(/"/g, '""')}"`)
        .join(","),
    );
  ...
  return (
    <Layout>
        ...
        <div className="card mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold">
              Sites & Bâtiments – Gestion Sécurité
            </h1>
            <p className="text-sm text-muted mt-2">
              Accédez aux sites pour consulter bâtiments, espaces, équipements
              et gérer vos audits.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3">
            <button
              type="button"
              aria-label="Créer un site"
              onClick={handleCreate}
              className="btn-primary px-4 py-2 rounded-md text-sm w-full sm:w-auto"
            >
              Créer un site
            </button>
            <button
              type="button"
              aria-label="Exporter la liste de sites en CSV"
              onClick={exportCsv}
              className="btn px-3 py-2 rounded-md text-sm border border-border w-full sm:w-auto"
            >
              Exporter CSV
            </button>
            <Link
              aria-label="Voir la carte France"
              to="/map-france"
              className="btn-ghost px-3 py-2 rounded-md text-sm w-full sm:w-auto"
            >
              Voir la carte
            </Link>
          </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        />
        <KpiCard
          title="Bâtiments total"
          value={kpis.buildings}
          icon={<span>🏗️</span>}
        />
        <KpiCard
          title="Espaces total"
      ...
      <section>
        {visible.length === 0 ? (
          <div className="card text-center py-10">
            ...
            <p className="text-sm text-muted mt-2">
              Vous pouvez créer votre premier site pour commencer vos audits.
            </p>
            <div className="mt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {visible.map((site) => (
              <div
                key={site.id}
                className="card-elevated flex flex-col justify-between"
                ...
                <div>
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="mt-2 inline-block text-xs px-2 py-1 rounded-md bg-green-100 text-green-800">
                        Actif
                      </div>
                    </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div className="p-2 bg-muted/30 rounded-md">
                      ...
                      <div className="font-semibold">
                        {countsMap[site.id]?.spaces ?? 0}
                      </div>
                    </div>
                    <div className="p-2 bg-muted/30 rounded-md">
                      <div className="text-xs text-muted">Équipements</div>
                      <div className="font-semibold">
                        {countsMap[site.id]?.equipments ?? 0}
                      </div>
                    </div>
                    <div className="p-2 bg-muted/30 rounded-md">
