...
export default function Synthese() {
  ...
  const percentOk = Math.round(
    (equipments.filter((e) => e.state === "OK").length /
      (equipments.length || 1)) *
      100,
  );
  const percentNon = Math.round(
  ...
  const pieData = [
    ...
    {
      label: "À contrôler",
      value: equipments.filter((e) => e.state === "A_CONTROLER").length,
    },
    {
      label: "Non conforme",
      value: equipments.filter((e) => e.state === "NON_CONFORME").length,
    },
    {
      label: "Obsolète",
      value: equipments.filter((e) => e.state === "OBSOLETE").length,
    },
    {
      label: "Absent",
      value: equipments.filter((e) => e.state === "ABSENT").length,
    },
  ];
  ...
  const exportCsv = () => {
    if (view === "equipments") {
      ...
      const csv = [
        "building,space,category,name,reference,state,comment",
        ...rows,
      ].join("\n");
      ...
      const csv = [
        "building,space,title,level,probability,impact,recommendation",
        ...rows,
      ].join("\n");
      const blob = new Blob([csv], { type: "text/csv" });
      const url = URL.createObjectURL(blob);
    ...
    <Layout>
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">
            Synthèse Globale de la Sécurité
          </h1>
          <p
            className="text-sm mt-1"
            style={{ color: "var(--text)", fontWeight: 600 }}
          >
            Vue consolidée des équipements, risques et statuts pour l’ensemble
            du périmètre sélectionné.
          </p>
        </div>
      ...
      <section className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3 py-2 rounded-md ${view === "equipments" ? "bg-primary text-white" : "border border-border"}`}
          >
            Équipements
          </button>
          <button
            onClick={() => setView("risks")}
            className={`px-3 py-2 rounded-md ${view === "risks" ? "bg-destructive text-white" : "border border-border"}`}
          >
            Risques
          </button>
        </div>
        <div className="text-sm text-muted">
          Vue: {view === "equipments" ? "Équipements" : "Risques"} • Résultats:{" "}
          {view === "equipments"
            ? filteredEquipments.length
      ...
      <section>
        {view === "equipments" ? (
          <div>
            <EquipmentTable
              items={filteredEquipments.slice(
                (page - 1) * pageSize,
                page * pageSize,
              )}
              onEdit={() => {
                alert("Édition équipement (mock)");
              }}
              onDelete={(id) => {
                if (window.confirm("Supprimer équipement ?")) {
                  builder
                    .deleteEquipment(id)
                    .then(() =>
          ...
          <div>
            <RiskTable
              items={filteredRisks.slice(
                (page - 1) * pageSize,
                page * pageSize,
              )}
              onEdit={(r) => {
                alert("Modifier risque (mock)");
              }}
              onDelete={(id) => {
                if (window.confirm("Supprimer risque ?")) {
                  builder
                    .deleteRisk(id)
                    .then(() =>
              ...
              onCreateAction={(riskId) => {
                alert("Créer action pour " + riskId);
              }}
            />
            <div className="mt-3 flex items-center justify-between">
              <div
            ...
            <div className="mt-2 text-sm text-muted">
              {equipments.filter((e) => e.state === "NON_CONFORME").length >
                ...
                >
                  ⚠️{" "}
                  {equipments.filter((e) => e.state === "NON_CONFORME").length}{" "}
                  équipements non conformes détectés
                </div>
              )}
              {risks.filter((r) => r.level === "CRITIQUE").length > 0 && (
                <div style={{ color: "#000", fontWeight: 700 }}>
                  🔥 {risks.filter((r) => r.level === "CRITIQUE").length}{" "}
                  risques critiques — Vérifier immédiatement les zones
                  concernées.
                </div>
              )}
            </div>
