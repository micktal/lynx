...
export default function ActionPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const atts = (await builder.fetchAttachments()).filter(
        (att) =>
          att.auditId === a.riskId ||
          att.riskId === a.riskId ||
          (att.uploadedBy && att.uploadedBy === a.id) ||
          (att as any).actionId === a.id,
      );
      setAttachments(atts);
  ...
  const statusBadge = (s?: ActionItem["status"]) => {
    switch (s) {
      ...
      case "EN_COURS":
        return (
          <span className="px-2 py-1 rounded bg-orange-400">EN_COURS</span>
        );
      case "CLOTUREE":
        return (
  ...
  const handleDelete = async () => {
    ...
    else if (risk && risk.spaceId)
      window.location.href = `/space/${risk.spaceId}`;
    else window.location.href = "/";
  };

  const handleToggleStatus = async (newStatus: ActionItem["status"]) => {
    if (!action) return;
    const updated = await builder.updateAction(action.id, {
      status: newStatus,
    });
    if (updated) setAction(updated);
  };
      ...
      <Layout>
        <div className="card">
          ...
          <p className="text-sm text-muted mt-2">
            L'action demandée est introuvable.
          </p>
        </div>
      </Layout>
    );
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <div className="text-sm text-muted mt-1">
            Risque:{" "}
            {risk ? (
              <Link to={`/risk/${risk.id}`} className="underline">
                {risk.title}
              </Link>
            ) : (
              "-"
          ...
          <div className="text-sm text-muted mt-1">
            Responsable: {action.ownerId || "-"} • Échéance:{" "}
            {action.dueDate || "-"}
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => {
              setToDelete({ type: "action", id: action.id });
              setConfirmOpen(true);
            }}
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => handleToggleStatus("EN_COURS")}
              className="px-3 py-2 rounded-md border border-border"
            >
              Marquer en cours
            </button>
            <button
              onClick={() => handleToggleStatus("CLOTUREE")}
              className="px-3 py-2 rounded-md brand-btn"
            >
              Clôturer
            </button>
          </div>
      ...
      <section className="mb-6">
        ...
        <div className="card p-4">
          ...
          {logs.length === 0 ? (
            ...
            >
              Aucun événement enregistré pour cette action.
            </div>
          ) : (
            <div>
              {/* render timeline */}
