...
export default function MapFranceAirbnb(): JSX.Element {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      mapRef.current = L.map("map-airbnb-root", { preferCanvas: true }).setView(
        [46.5, 2.5],
        5,
      );
      L.tileLayer(
    ...
    function render() {
      ...
      const filtered = (sites || []).filter(
        (s) => s.lat && s.lng && (s.scoreCriticite || 0) >= minScore,
      );
        ...
        iconCreateFunction: function (cluster: any) {
          ...
          const avg =
            Math.round(
              (cluster
                .getAllChildMarkers()
                .reduce((a: number, m: any) => a + (m.options._score || 0), 0) /
                Math.max(1, count)) *
                10,
            ) / 10;
          ...
          // size rule
          const size =
            count >= 200 ? 70 : count >= 50 ? 56 : count >= 10 ? 44 : 32;
      ...
      for (const s of filtered) {
        ...
        const icon = L.divIcon({
          html,
          className: "airbnb-marker-wrap",
          iconSize: [size, size],
          iconAnchor: [size / 2, size / 2],
        });
        const marker = L.marker([s.lat!, s.lng!], { icon, _score: score });
        const clientName =
          clients.find((c: any) => c.id === s.clientId)?.name || "—";
        marker.bindTooltip(
          `<strong>${s.name}</strong><br/>Client: ${clientName}<br/>Progression: ${s.progressionTravaux || 0}%<br/>Risques critiques: ${s.nbRisquesCritiques || 0}<br/>Actions en retard: ${s.nbActionsEnRetard || 0}`,
        );
      ...
      mcg.on("clusterclick", (ev: any) => {
        try {
          ...
          // animate zoom
          mapRef.current.flyToBounds(ev.layer.getBounds().pad(0.4), {
            duration: 0.6,
            easeLinearity: 0.25,
          });
        } catch (e) {}
      ...
      if (heatOn) {
        ...
        const heatPoints = filtered.map((s) => [
          s.lat!,
          s.lng!,
          Math.max(0.05, (s.scoreCriticite || 0) / max),
        ]);
        try {
          // @ts-ignore
          ...
          setTimeout(() => {
            const canv = mapRef.current
              .getContainer()
              .querySelector(".leaflet-heatmap-layer, canvas");
            if (canv && (canv as HTMLElement).style)
              (canv as HTMLElement).classList.add("heat-breathe");
          }, 50);
        } catch (e) {
          console.warn(e);
        }
  ...
  return (
    <Layout>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-bold">
            Carte France — Clusters animés (Airbnb-like)
          </h1>
          <div className="text-sm text-muted">
            Navigation multi-niveaux, clusters animés, heatmap
          </div>
        </div>
      ...
      <div
        id="map-airbnb-root"
        style={{ height: 760, borderRadius: 16, overflow: "hidden" }}
      />

      {selectedSite && (
        <div
          className="fixed right-6 top-20 w-96 bg-card p-6 rounded-3xl shadow-xl airbnb-drawer"
        ...
        >
          ...
          <div className="mt-4 flex gap-2">
            <a className="btn-premium" href={`/site/${selectedSite.id}`}>
              Voir la fiche site
            </a>
            <a className="btn-ghost" href={`/audit/${selectedSite.id}`}>
              Voir audit
            </a>
            <a className="btn-ghost" href={`/actions?site=${selectedSite.id}`}>
              Voir actions
            </a>
          </div>
        </div>
