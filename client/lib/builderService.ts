...
let MOCK_CLIENTS: any[] = [
  {
    id: "client_1",
    name: "Client Demo Retail",
    logoUrl: "",
    industry: "Retail",
    contactName: "Paul Client",
    contactEmail: "paul@client.com",
    active: true,
  },
];
...
const MOCK_SITES: Site[] = [
  {
    ...
    contactEmail: "alice@ex.com",
    lat: 48.8566,
    lng: 2.3522,
    status: "IN_PROGRESS",
    scoreCriticite: 0,
    progressionTravaux: 72,
    regionCode: "IDF",
    departmentCode: "75",
    nbRisquesCritiques: 0,
    nbActionsEnRetard: 0,
  ...
  },
  {
    ...
    city: "Lyon",
    country: "France",
    contactName: "Marc Legrand",
    contactEmail: "marc@ex.com",
    lat: 45.764,
    lng: 4.8357,
    status: "NOT_STARTED",
    scoreCriticite: 0,
    progressionTravaux: 10,
    regionCode: "ARA",
    departmentCode: "69",
    nbRisquesCritiques: 0,
    nbActionsEnRetard: 0,
  ...
  },
  {
    ...
    contactEmail: "sophie@ex.com",
    lat: 44.8378,
    lng: -0.5792,
    status: "FINISHED",
    scoreCriticite: 0,
    progressionTravaux: 100,
    regionCode: "NAQ",
    departmentCode: "33",
    nbRisquesCritiques: 0,
    nbActionsEnRetard: 0,
...
let MOCK_SPACES: Space[] = [
  ...
  },
  {
    ...
    name: "Salle Serveurs",
    code: "SS1",
    description: "Salle serveurs critique",
    floor: 1,
    importance: 5,
    accessLevel: "Très restreint",
  },
];
...
let MOCK_RISKS: Risk[] = [
  {
    ...
    spaceId: "s_1",
    equipmentId: "e_1",
    title: "Câblage défectueux",
    description: "Câblage apparent",
    probability: 3,
    impact: 3,
    level: "MOYEN",
    recommendation: "Sécuriser les câbles",
  },
  {
    id: "r_2",
    auditId: undefined,
    ...
    description: "Caméra non connectée",
    probability: 4,
    impact: 2,
    level: "IMPORTANT",
    recommendation: "Remplacer la caméra",
  },
  {
    id: "r_3",
    auditId: undefined,
    ...
    spaceId: "s_5",
    equipmentId: "e_3",
    title: "Détecteur non conforme",
    description: "Mesure erronée",
    probability: 5,
    impact: 5,
    level: "CRITIQUE",
    recommendation: "Intervention immédiate",
  },
];
...
let MOCK_AUDITS: Audit[] = [
  ...
  },
  {
    ...
    templateId: "t_2",
    status: "in_progress",
    title: "Audit sécurité infra",
    auditorId: "u_2",
    scheduledAt: "2024-10-01",
  },
];
...
let MOCK_ACTIONS: ActionItem[] = [
  {
    id: "act_1",
    riskId: "r_1",
    title: "Protéger câbles",
    description: "Passer sous goulotte",
    ownerId: "u_2",
    dueDate: "2024-12-01",
    status: "OUVERTE",
  },
  {
    ...
    description: "Commander et remplacer",
    ownerId: "u_3",
    dueDate: "2024-08-15",
    status: "EN_COURS",
  },
...
export async function fetchRegions(): Promise<Region[]> {
  return structuredClone(
    MOCK_REGIONS.map((r) => ({ ...r, geojsonUrl: REGIONS_GEOJSON_CDN })),
  );
}
export async function fetchDepartments(): Promise<Department[]> {
  return structuredClone(
    MOCK_DEPARTMENTS.map((d) => ({
      ...d,
      geojsonUrl: DEPARTMENTS_GEOJSON_CDN,
    })),
  );
}
export async function createClient(c: Partial<any>): Promise<any> {
  const newC = {
    id: `client_${Date.now()}`,
    name: c.name || "Nouveau client",
    logoUrl: c.logoUrl,
    industry: c.industry,
...
export async function updateClient(
  ...
  const idx = MOCK_CLIENTS.findIndex((x) => x.id === id);
  if (idx === -1) return null;
  MOCK_CLIENTS[idx] = { ...MOCK_CLIENTS[idx], ...patch };
  return structuredClone(MOCK_CLIENTS[idx]);
}
export async function deleteClient(id: string): Promise<boolean> {
  const idx = MOCK_CLIENTS.findIndex((x) => x.id === id);
  if (idx === -1) return false;
  MOCK_CLIENTS.splice(idx, 1); // optionally cleanup related sites
  return true;
}

let MOCK_CLIENT_USERS: any[] = [
  {
    id: "cu_1",
    userId: "u_client_1",
    clientId: "client_1",
...
export async function fetchClientUsers(clientId?: string): Promise<any[]> {
  return structuredClone(
    clientId
      ? MOCK_CLIENT_USERS.filter((u) => u.clientId === clientId)
      : MOCK_CLIENT_USERS,
  );
}
export async function createClientUser(u: Partial<any>): Promise<any> {
  const newU = {
    id: `cu_${Date.now()}`,
    userId: u.userId || `u_client_${Date.now()}`,
    clientId: u.clientId || "",
    role: u.role || "CLIENT_VIEW",
    permissions: u.permissions || "{}",
    active: typeof u.active === "boolean" ? u.active : true,
  };
  MOCK_CLIENT_USERS.unshift(newU);
  return structuredClone(newU);
}
export async function updateClientUser(
  id: string,
  patch: Partial<any>,
): Promise<any | null> {
  const idx = MOCK_CLIENT_USERS.findIndex((x) => x.id === id);
  if (idx === -1) return null;
  MOCK_CLIENT_USERS[idx] = { ...MOCK_CLIENT_USERS[idx], ...patch };
  return structuredClone(MOCK_CLIENT_USERS[idx]);
}
export async function deleteClientUser(id: string): Promise<boolean> {
  const idx = MOCK_CLIENT_USERS.findIndex((x) => x.id === id);
  if (idx === -1) return false;
  MOCK_CLIENT_USERS.splice(idx, 1);
  return true;
}
...
let MOCK_WORKFLOW_RULES: WorkflowRule[] = [
  {
    ...
    notificationTemplate:
      "Risque critique détecté : {{risk.title}} - assigné au chef de site",
  ...
  },
  {
    ...
    notificationTemplate:
      "Une action liée à un équipement incendie vous a été assignée : {{action.title}}",
    active: true,
    createdAt: new Date().toISOString(),
  },
];
...
export async function fetchActivityLogs(): Promise<ActivityLog[]> {
  return structuredClone(
    MOCK_ACTIVITY_LOGS.sort((a, b) => b.timestamp.localeCompare(a.timestamp)),
  );
}

export async function fetchActivityLogsForEntity(
  entityType: string,
  entityId: string,
): Promise<ActivityLog[]> {
  return structuredClone(
    MOCK_ACTIVITY_LOGS.filter(
      (l) => l.entityType === entityType && l.entityId === entityId,
    ).sort((a, b) => b.timestamp.localeCompare(a.timestamp)),
  );
}

export async function fetchActivityLogsWithFilters(filters: {
  from?: string;
  to?: string;
  entityType?: string;
  operation?: string;
  ...
  return structuredClone(
    MOCK_ACTIVITY_LOGS.filter((l) => {
      if (filters.entityType && l.entityType !== filters.entityType)
        return false;
      if (filters.operation && l.operation !== (filters.operation as any))
        return false;
...
export async function fetchBuildingPlans(
  buildingId?: string,
): Promise<BuildingPlan[]> {
  if (buildingId)
    return structuredClone(
      MOCK_BUILDING_PLANS.filter((p) => p.buildingId === buildingId),
    );
  return structuredClone(MOCK_BUILDING_PLANS);
}

export async function createBuildingPlan(
  p: Partial<BuildingPlan>,
): Promise<BuildingPlan> {
...
export async function updateBuildingPlan(
  id: string,
  patch: Partial<BuildingPlan>,
): Promise<BuildingPlan | null> {
...
export async function fetchPlanAreas(
  buildingPlanId: string,
): Promise<PlanArea[]> {
  return structuredClone(
    MOCK_PLAN_AREAS.filter((a) => a.buildingPlanId === buildingPlanId),
  );
}
...
export async function updatePlanArea(
  id: string,
  patch: Partial<PlanArea>,
): Promise<PlanArea | null> {
...
export async function fetchPlanMarkers(
  buildingPlanId: string,
): Promise<PlanMarker[]> {
  return structuredClone(
    MOCK_PLAN_MARKERS.filter((m) => m.buildingPlanId === buildingPlanId),
  );
}

export async function createPlanMarker(
  m: Partial<PlanMarker>,
): Promise<PlanMarker> {
...
export async function updatePlanMarker(
  id: string,
  patch: Partial<PlanMarker>,
): Promise<PlanMarker | null> {
...
let MOCK_INCIDENTS: Incident[] = [
  {
    ...
    buildingId: "b_4",
    spaceId: "s_5",
    description: "Fumée détectée salle serveurs",
    reportedBy: "u_2",
    createdAt: new Date().toISOString(),
  },
];
let MOCK_AGENTS: Agent[] = [
  {
    ...
    role: "agent",
    status: "ON_PATROL",
    lastKnownPosition: { lat: 48.8566, lng: 2.3522 },
  ...
  },
  {
    ...
    role: "agent",
    status: "AVAILABLE",
    lastKnownPosition: { lat: 44.8378, lng: -0.5792 },
    lastCheckIn: new Date().toISOString(),
    siteId: "site_3",
    battery: 56,
  },
...
export async function fetchIncidents(): Promise<Incident[]> {
  return structuredClone(MOCK_INCIDENTS);
}
export async function fetchIncidentsForSite(
  siteId: string,
): Promise<Incident[]> {
  return structuredClone(MOCK_INCIDENTS.filter((i) => i.siteId === siteId));
}
export async function createIncident(i: Partial<Incident>): Promise<Incident> {
  const newI: Incident = {
    id: `inc_${Date.now()}`,
    type: (i.type as any) || "autre",
  ...
  MOCK_INCIDENTS.unshift(newI);
  await createSupervisionEvent({
    eventType: "ALERT",
    entityType: "incident",
    entityId: newI.id,
    description: `Incident ${newI.type} créé`,
    timestamp: new Date().toISOString(),
  });
  return structuredClone(newI);
}
export async function updateIncident(
  ...
  return structuredClone(MOCK_INCIDENTS[idx]);
}
...
export async function updateAgent(
  ...
  const idx = MOCK_AGENTS.findIndex((x) => x.id === id);
  if (idx === -1) return null;
  MOCK_AGENTS[idx] = { ...MOCK_AGENTS[idx], ...patch };
  return structuredClone(MOCK_AGENTS[idx]);
}
export async function createAgentEvent(
  e: Partial<AgentEvent>,
): Promise<AgentEvent> {
  const newE: AgentEvent = {
    id: `ae_${Date.now()}`,
    agentId: e.agentId || "",
    eventType: (e.eventType as any) || "CHECK_IN",
    timestamp: e.timestamp || new Date().toISOString(),
    data: e.data,
  };
  MOCK_AGENT_EVENTS.unshift(newE);
  await createSupervisionEvent({
    eventType: "INFO",
    entityType: "agent",
    entityId: newE.agentId,
    description: `Agent event ${newE.eventType}`,
...
export async function fetchAgentEvents(
  ...
  return structuredClone(
    agentId
      ? MOCK_AGENT_EVENTS.filter((a) => a.agentId === agentId)
      : MOCK_AGENT_EVENTS,
  );
}

export async function addLocationPing(
  p: Partial<LocationPing>,
): Promise<LocationPing> {
  const newP: LocationPing = {
    id: `lp_${Date.now()}`,
    agentId: p.agentId || "",
    lat: p.lat || 0,
    lng: p.lng || 0,
  ...
  MOCK_LOCATION_PINGS.unshift(newP); // update agent position
  const agentIdx = MOCK_AGENTS.findIndex((a) => a.id === newP.agentId);
  if (agentIdx !== -1) {
    MOCK_AGENTS[agentIdx].lastKnownPosition = { lat: newP.lat, lng: newP.lng };
    MOCK_AGENTS[agentIdx].lastCheckIn = newP.timestamp;
  }
  return structuredClone(newP);
}
export async function fetchLocationPings(
  agentId?: string,
): Promise<LocationPing[]> {
  return structuredClone(
    agentId
      ? MOCK_LOCATION_PINGS.filter((p) => p.agentId === agentId)
      : MOCK_LOCATION_PINGS,
  );
}

export async function createSupervisionEvent(
  ev: Partial<SupervisionEvent>,
): Promise<SupervisionEvent> {
  const newEv: SupervisionEvent = {
    id: `sev_${Date.now()}`,
    eventType: (ev.eventType as any) || "SYSTEM",
  ...
  MOCK_SUPERVISION_EVENTS.unshift(newEv);
  return structuredClone(newEv);
}
export async function fetchSupervisionEvents(): Promise<SupervisionEvent[]> {
  return structuredClone(MOCK_SUPERVISION_EVENTS);
}
...
let MOCK_AUDIT_TEMPLATES: AuditTemplate[] = [
  {
    id: "at_1",
    name: "Template Sécurité Incendie",
    description: "Checklist incendie standard",
    auditType: "incendie",
    active: true,
    createdAt: new Date().toISOString(),
  },
...
let MOCK_AUDIT_QUESTIONS: AuditQuestion[] = [
  {
    id: "q_1",
    subcategoryId: "asc_1",
    label: "Extincteurs accessibles ?",
    helpText: "Vérifier que l'extincteur est accessible et visible.",
    type: "yesno",
    critical: true,
    defaultRiskLevel: "IMPORTANT",
  ...
  },
  {
    id: "q_2",
    subcategoryId: "asc_2",
    label: "Détecteur opérationnel ?",
    helpText: "Vérifier le témoin et les tests.",
    type: "yesno",
    critical: true,
    defaultRiskLevel: "CRITIQUE",
    defaultActionRequired: true,
    order: 1,
  },
];
...
export async function updateAuditTemplate(
  ...
  const idx = MOCK_AUDIT_TEMPLATES.findIndex((x) => x.id === id);
  if (idx === -1) return null;
  MOCK_AUDIT_TEMPLATES[idx] = { ...MOCK_AUDIT_TEMPLATES[idx], ...patch };
  return structuredClone(MOCK_AUDIT_TEMPLATES[idx]);
}
export async function deleteAuditTemplate(id: string): Promise<boolean> {
  const idx = MOCK_AUDIT_TEMPLATES.findIndex((x) => x.id === id);
  if (idx === -1) return false;
  MOCK_AUDIT_TEMPLATES.splice(idx, 1);
  return true;
}

export async function fetchCategoriesForTemplate(
  templateId: string,
): Promise<AuditCategory[]> {
  return structuredClone(
    MOCK_AUDIT_CATEGORIES.filter((c) => c.templateId === templateId),
  );
}
export async function createCategory(
  c: Partial<AuditCategory>,
): Promise<AuditCategory> {
  const newC: AuditCategory = {
    id: `ac_${Date.now()}`,
    templateId: c.templateId || "",
    name: c.name || "Nouvelle catégorie",
    order: c.order,
  };
  MOCK_AUDIT_CATEGORIES.unshift(newC);
  return structuredClone(newC);
}
export async function fetchSubcategoriesForCategory(
  categoryId: string,
): Promise<AuditSubcategory[]> {
  return structuredClone(
    MOCK_AUDIT_SUBCATS.filter((s) => s.categoryId === categoryId),
  );
}
export async function createSubcategory(
  s: Partial<AuditSubcategory>,
): Promise<AuditSubcategory> {
  const newS: AuditSubcategory = {
    id: `asc_${Date.now()}`,
    categoryId: s.categoryId || "",
    name: s.name || "Nouvelle sous-catégorie",
    order: s.order,
  };
  MOCK_AUDIT_SUBCATS.unshift(newS);
  return structuredClone(newS);
}

export async function fetchQuestionsForSubcategory(
  subcategoryId: string,
): Promise<AuditQuestion[]> {
  return structuredClone(
    MOCK_AUDIT_QUESTIONS.filter((q) => q.subcategoryId === subcategoryId),
  );
}
export async function createQuestion(
  q: Partial<AuditQuestion>,
): Promise<AuditQuestion> {
  const newQ: AuditQuestion = {
    id: `q_${Date.now()}`,
    subcategoryId: q.subcategoryId || "",
    label: q.label || "Nouvelle question",
    helpText: q.helpText,
    type: (q.type as any) || "text",
    critical: !!q.critical,
    defaultRiskLevel: q.defaultRiskLevel,
    defaultActionRequired: !!q.defaultActionRequired,
    order: q.order,
  };
  MOCK_AUDIT_QUESTIONS.unshift(newQ);
  return structuredClone(newQ);
}

export async function fetchComplianceOptionsForQuestion(
  questionId: string,
): Promise<ComplianceOption[]> {
  return structuredClone(
    MOCK_COMPLIANCE_OPTIONS.filter((o) => o.questionId === questionId),
  );
}
export async function createComplianceOption(
  o: Partial<ComplianceOption>,
): Promise<ComplianceOption> {
  const newO: ComplianceOption = {
    id: `co_${Date.now()}`,
    questionId: o.questionId || "",
    label: o.label || "Option",
    riskLevel: o.riskLevel,
    createsAction: !!o.createsAction,
    createsRisk: !!o.createsRisk,
  };
  MOCK_COMPLIANCE_OPTIONS.unshift(newO);
  return structuredClone(newO);
}

export async function fetchGeneratedQuestionsForAudit(
  auditId: string,
): Promise<GeneratedAuditQuestion[]> {
  return structuredClone(
    MOCK_GENERATED_QUESTIONS.filter((g) => g.auditId === auditId),
  );
}

// createAudit: generates audit and creates generated questions per space based on template
export async function createAudit(
  a: Partial<Audit> & { templateId?: string; spaces?: string[] },
): Promise<Audit> {
  const newA: Audit = {
    id: `a_${Date.now()}`,
    siteId: a.siteId || "",
  ...
  if (a.templateId && a.spaces && a.spaces.length > 0) {
    ...
    const cats = MOCK_AUDIT_CATEGORIES.filter(
      (c) => c.templateId === a.templateId,
    ).map((c) => c.id);
    const subcats = MOCK_AUDIT_SUBCATS.filter((s) =>
      cats.includes(s.categoryId),
    ).map((s) => s.id);
    const qs = MOCK_AUDIT_QUESTIONS.filter((q) =>
      subcats.includes(q.subcategoryId),
    );
    for (const spaceId of a.spaces) {
      for (const q of qs) {
        const gq: GeneratedAuditQuestion = {
          id: `gq_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`,
          auditId: newA.id,
          questionId: q.id,
          spaceId,
          value: undefined,
...
export async function answerGeneratedQuestion(
  gqId: string,
  payload: Partial<GeneratedAuditQuestion>,
): Promise<GeneratedAuditQuestion | null> {
  const idx = MOCK_GENERATED_QUESTIONS.findIndex((g) => g.id === gqId);
  if (idx === -1) return null;
  MOCK_GENERATED_QUESTIONS[idx] = {
    ...MOCK_GENERATED_QUESTIONS[idx],
    ...payload,
  };
  ...
  const opt = MOCK_COMPLIANCE_OPTIONS.find(
    (o) => o.questionId === gq.questionId && o.label === payload.value,
  );
  if (opt) {
    if (opt.createsRisk) {
      const risk = await createRisk({
        title: q?.label || "Auto risk",
        description: q?.helpText,
        level: opt.riskLevel || "MOYEN",
        siteId: undefined,
    ...
    if (opt.createsAction) {
      const action = await createAction({
        riskId: MOCK_GENERATED_QUESTIONS[idx].autoRiskId || "",
        title: `Action auto: ${q?.label || ""}`,
        status: "OUVERTE",
      });
...
export async function fetchNotificationsForUser(
  ...
  return structuredClone(
    MOCK_NOTIFICATIONS.filter((n) => n.userId === userId).sort((a, b) =>
...
export async function deleteBuilding(id: string): Promise<boolean> {
  ...
  MOCK_EQUIP = MOCK_EQUIP.filter((eq) =>
    MOCK_SPACES.some((sp) => sp.id === eq.spaceId),
  );
  return true;
}
...
export async function countBySite() {
  ...
  const perSite = sites.map((site) => {
    ...
    const siteSpaces = spaces.filter((sp) =>
      siteBuildings.some((b) => b.id === sp.buildingId),
    );
    const siteEquip = equipments.filter((eq) =>
      siteSpaces.some((sp) => sp.id === eq.spaceId),
    );
    return {
      siteId: site.id,
...
export async function countStatsForSite(siteId: string) {
  ...
  const spaces = MOCK_SPACES.filter((sp) =>
    buildings.some((b) => b.id === sp.buildingId),
  );
  const equipments = MOCK_EQUIP.filter((eq) =>
    spaces.some((s) => s.id === eq.spaceId),
  );
  ...
  const perBuilding = buildings.map((b) => {
    ...
    const bEquip = MOCK_EQUIP.filter((eq) =>
      bSpaces.some((s) => s.id === eq.spaceId),
    );
    const bRisks = MOCK_RISKS.filter((r) => r.buildingId === b.id);
    const audits = MOCK_AUDITS.filter((a) => a.buildingId === b.id).sort(
      (a, b2) => {
...
export async function countStatsForSpace(spaceId: string) {
  ...
  const actions = MOCK_ACTIONS.filter((a) =>
    risks.some((r) => r.id === a.riskId),
  );
  ...
  return {
    ...
    perRisk: risks.map((r) => ({
      riskId: r.id,
      actions: MOCK_ACTIONS.filter((a) => a.riskId === r.id),
    })),
  };
}
...
export async function fetchActionsForRisks(
  riskIds: string[],
): Promise<ActionItem[]> {
  return structuredClone(
    MOCK_ACTIONS.filter((a) => riskIds.includes(a.riskId)),
  );
}
...
export async function recalcSiteScores(): Promise<void> {
  for (const site of MOCK_SITES) {
    ...
    const now = new Date();
    const nbActionsLate = actionsForSite.filter(
      (a) => a.dueDate && new Date(a.dueDate) < now && a.status !== "CLOTUREE",
    ).length;
    const nbIncidentsOpen = MOCK_INCIDENTS.filter(
      (i) => i.siteId === site.id && i.status === "OPEN",
    ).length;
    const heatValue =
      nbCrit * 5 + nbImportant * 3 + nbActionsLate * 4 + nbIncidentsOpen * 2;
...
export async function writeDataLakeRecord(
  r: Partial<DataLakeRecord>,
): Promise<DataLakeRecord> {
...
export async function fetchDataLakeRecords(
  filters: {
    entityType?: string;
    entityId?: string;
    clientId?: string;
  ...
  return structuredClone(
    MOCK_DATA_LAKE.filter((rec) => {
      if (filters.entityType && rec.entityType !== filters.entityType)
        return false;
...
export async function exportDataLakeAsCsv(
  filters: {
    entityType?: string;
    entityId?: string;
    clientId?: string;
  ...
  const rows: string[] = [
    "id,entityType,entityId,timestamp,changeType,changedBy,clientId,snapshot",
  ];
...
export async function purgeDataLakeOlderThan(
  months: number,
  clientId?: string,
...
// Time-series
export async function writeTimeSeriesMetric(
  m: Partial<TimeSeriesMetric>,
...
export async function fetchTimeSeries(
  ...
  return structuredClone(
    MOCK_TIME_SERIES.filter((t) => {
      if (filters.metricType && t.metricType !== filters.metricType)
        return false;
      if (filters.siteId && t.siteId !== filters.siteId) return false;
      if (filters.buildingId && t.buildingId !== filters.buildingId)
        return false;
...
export async function fetchDataLakeStats(): Promise<{
  totalRecords: number;
  ...
  const avg = total
    ? Math.round(
        MOCK_DATA_LAKE.reduce(
          (s, r) => s + ((r.snapshot || "").length || 0),
          0,
        ) / total,
      )
    : 0;
...
export async function fetchDataLakeForEntity(
  entityType: string,
  entityId: string,
): Promise<DataLakeRecord[]> {
...
let MOCK_PERMISSIONS: Permission[] = [
  ...
  // client_view limited
  {
    id: "perm_client_view_risks",
    roleId: "role_client_view",
    resource: "risk",
    action: "VIEW",
    allowed: true,
...
let MOCK_USER_EXTENDED: UserExtended[] = [
  {
    id: "ue_1",
    userId: "u_1",
    roleId: "role_auditeur",
    clientId: "client_1",
    restrictedTo: JSON.stringify({ sites: ["site_1"] }),
  },
...
let MOCK_RULES: RuleEngineRule[] = [
  {
    ...
    onlyRoles: ["MANAGER", "ADMIN"],
    description: "Only managers/admin can update actions for critical risks",
    active: true,
  },
];
...
export async function createRole(r: Partial<Role>): Promise<Role> {
  const newR: Role = {
    id: `role_${Date.now()}`,
    name: r.name || "NEW_ROLE",
    description: r.description,
    level: r.level || 1,
    isSystem: !!r.isSystem,
  };
  MOCK_ROLES.unshift(newR);
  return structuredClone(newR);
}
export async function updateRole(
  ...
  const idx = MOCK_ROLES.findIndex((x) => x.id === id);
  if (idx === -1) return null;
  MOCK_ROLES[idx] = { ...MOCK_ROLES[idx], ...patch };
  return structuredClone(MOCK_ROLES[idx]);
}
export async function deleteRole(id: string): Promise<boolean> {
  const idx = MOCK_ROLES.findIndex((x) => x.id === id);
  if (idx === -1) return false;
  if (MOCK_ROLES[idx].isSystem) return false;
...
export async function fetchPermissions(roleId?: string): Promise<Permission[]> {
  return structuredClone(
    roleId
      ? MOCK_PERMISSIONS.filter((p) => p.roleId === roleId)
      : MOCK_PERMISSIONS,
  );
}
export async function createPermission(
  ...
  const newP: Permission = {
    ...
    roleId: p.roleId || "",
    resource: p.resource || "*",
    action: (p.action as any) || "VIEW",
    condition: p.condition,
    allowed: typeof p.allowed === "boolean" ? p.allowed : true,
  };
  MOCK_PERMISSIONS.unshift(newP);
  return structuredClone(newP);
}
export async function updatePermission(
  ...
  const idx = MOCK_PERMISSIONS.findIndex((x) => x.id === id);
  if (idx === -1) return null;
  MOCK_PERMISSIONS[idx] = { ...MOCK_PERMISSIONS[idx], ...patch };
  return structuredClone(MOCK_PERMISSIONS[idx]);
}
...
export async function fetchUserExtended(
  ...
  return structuredClone(
    userId
      ? MOCK_USER_EXTENDED.filter((u) => u.userId === userId)
      : MOCK_USER_EXTENDED,
  );
}
export async function upsertUserExtended(
  u: Partial<UserExtended>,
): Promise<UserExtended> {
  const existing = MOCK_USER_EXTENDED.find((x) => x.userId === u.userId);
  if (existing) {
    existing.roleId = u.roleId || existing.roleId;
    existing.clientId = u.clientId || existing.clientId;
    existing.siteId = u.siteId || existing.siteId;
    existing.restrictedTo = u.restrictedTo || existing.restrictedTo;
    return structuredClone(existing);
  }
  const newU: UserExtended = {
    id: `ue_${Date.now()}`,
    userId: u.userId || "",
    roleId: u.roleId || "",
    clientId: u.clientId,
    siteId: u.siteId,
    restrictedTo: u.restrictedTo,
  };
  MOCK_USER_EXTENDED.unshift(newU);
  return structuredClone(newU);
}

// Security logs
export async function writeSecurityLog(
  entry: Partial<SecurityAuditLog>,
): Promise<SecurityAuditLog> {
  const newE: SecurityAuditLog = {
    id: `sec_${Date.now()}`,
    userId: entry.userId,
    action: entry.action || "",
...
function resolveValueToken(
  ...
  if (token.startsWith("userExt.")) {
    ...
    try {
      return userExt
        ? JSON.parse(userExt.restrictedTo || "{}")[key]
        : undefined;
    } catch (e) {
      return undefined;
    }
...
function evaluateCondition(
  ...
  try {
    ...
    const left = cond.field
      ? entity
        ? cond.field.indexOf(".") !== -1
          ? (() => {
              const parts = cond.field.split(".");
              let cur: any = entity;
              for (const p of parts) {
                cur = cur ? (p in cur ? cur[p] : undefined) : undefined;
              }
              return cur;
            })()
          : entity[cond.field]
    ...
    const right =
      ...
      (rightToken.startsWith("currentUser.") ||
        rightToken.startsWith("userExt.") ||
        rightToken.indexOf(".") !== -1)
        ? resolveValueToken(rightToken as string, currentUser, userExt, entity)
        : rightToken;
...
export async function hasPermission(
  ...
  const perms = role
    ? MOCK_PERMISSIONS.filter((p) => p.roleId === role.id)
    : [];
  // wildcard resource permissions
  const wildcardPerms = MOCK_PERMISSIONS.filter(
    (p) => p.resource === "*" && p.roleId === role?.id,
  );
  ...
  for (const p of allPerms) {
    if (
      p.action === action &&
      p.allowed === false &&
      evaluateCondition(p.condition, entity, currentUser, uext)
    )
      return false;
  }
  // check allows
  ...
  for (const rule of rules) {
    ...
    if (
      rule.condition &&
      !evaluateCondition(rule.condition, entity, currentUser, uext)
    )
      return false;
  }
  // default: deny
