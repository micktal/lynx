export type CurrentUser = { id: string; name: string; email?: string; role?: string } | null;
...
export async function login(email: string, password: string) {
  ...
  if (email === adminCred.email && password === adminCred.password) {
    user = { id: 'u_admin', name: 'Admin Demo', email: adminCred.email, role: 'ADMIN' };
  } else if (email === userCred.email && password === userCred.password) {
    user = { id: 'u_1', name: 'User Demo', email: userCred.email, role: 'AUDITEUR' };
  } else {
