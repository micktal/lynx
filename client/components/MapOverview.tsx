...
export default function MapOverview({ sites, onOpen, stats, mode = 'heat' }: { sites: Site[]; onOpen: (id: string) => void; stats?: { siteId: string; score?: number; nonConform?: number; criticalRisks?: number; actionsOpen?: number }[]; mode?: 'heat' | 'list' }) {
  ...
  const posFor = (s: Site) => {
    if (minLat === maxLat || minLng === maxLng) return { left: '50%', top: '50%' };
  ...
  useEffect(()=>{
    ...
    el.forEach((e, i) => { (e as HTMLElement).style.opacity = '0'; setTimeout(()=>{ (e as HTMLElement).style.opacity = '1'; }, 60 * i); });
  }, [sites]);

  const totalCritical = (stats || []).reduce((acc, s) => acc + (s.criticalRisks || 0), 0);
  const totalActions = (stats || []).reduce((acc, s) => acc + (s.actionsOpen || 0), 0);
  const totalNonConform = (stats || []).reduce((acc, s) => acc + (s.nonConform || 0), 0);
    ...
    <div className="card relative overflow-hidden">
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setViewMode('heat')} className={`px-2 py-1 rounded ${viewMode==='heat' ? 'bg-primary text-white' : 'bg-transparent border border-border'}`}>Heatmap</button>
          <button onClick={() => setViewMode('list')} className={`px-2 py-1 rounded ${viewMode==='list' ? 'bg-primary text-white' : 'bg-transparent border border-border'}`}>Liste</button>
        </div>
      </div>

      <div className={`relative rounded-lg h-72 overflow-hidden transition-all duration-300 ${viewMode==='list' ? 'bg-card' : 'bg-gradient-to-br from-sky-50 to-white'}`}>

        {/* heat spots */}
        {viewMode==='heat' && sites.map((s) => {
          ...
          return (
            <div key={`spot-${s.id}`} className="heat-spot pulse" style={{ position: 'absolute', left: pos.left, top: pos.top, width: size, height: size, transform: 'translate(-50%, -50%)', pointerEvents: 'none', background: gradient, borderRadius: '50%' }} />
          );
        })}
        ...
        {viewMode==='heat' ? (
          sites.map((s) => {
            const pos = posFor(s);
            return (
              <button key={`m-${s.id}`} onClick={() => onOpen(s.id)} title={s.name} className="absolute -translate-x-1/2 -translate-y-1/2 p-0 transform hover:scale-110 transition" style={{ left: pos.left, top: pos.top }}>
          ...
          <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-2 h-full overflow-auto">
            {sites.map(s => (
              <button key={`li-${s.id}`} onClick={() => onOpen(s.id)} className="text-left p-3 rounded border border-border bg-card hover:shadow flex items-center gap-3">
                <div className="w-10 h-10 rounded bg-primary/10 flex items-center justify-center text-primary font-bold">{s.name?.charAt(0) ?? ''}</div>
                <div className="truncate">
                  <div className="font-medium truncate">{s.name}</div>
                  <div className="text-xs text-muted truncate">{s.city}, {s.country}</div>
                </div>
                <div className="ml-auto text-sm text-muted">{(statsMap.get(s.id)?.criticalRisks || 0)} 🔥</div>
              </button>
            ))}
          </div>
        )}
        ...
        <div className="absolute right-4 top-4 w-56 space-y-2 z-10">
          <div className="p-3 rounded-lg bg-gradient-to-r from-red-600/10 to-red-400/6 border border-red-300/10">
            ...
            <div className="text-lg font-bold text-destructive animate-pulse-fast">{totalCritical}</div>
          </div>
          ...
          <div className="p-3 rounded-lg bg-gradient-to-r from-sky-50/6 to-sky-300/8 border border-border">
            ...
            <div className="text-lg font-bold text-destructive animate-pop-slow">{totalNonConform}</div>
          </div>
        </div>
