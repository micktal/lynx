...
export default function VisualSummary({pieData, barData}: {pieData: {label:string,value:number}[], barData:{label:string,critical:number,important:number,medium:number,low:number}[]}){
  ...
  pieData.forEach((d, i) => {
    ...
    const colors = ["#16a34a","#f59e0b","#ef4444","#f97316","#9ca3af"];
      ...
      <div className="card">
        ...
        <div className="flex items-center gap-4">
          <svg width="120" height="120" viewBox="0 0 120 120">
            {paths.map((p, i) => (
              <path key={i} d={p.path} fill={p.color}></path>
            ))}
          </svg>
          <div>
            {pieData.map((d) => (
              <div key={d.label} className="flex items-center gap-2 text-sm">
                <span className="w-3 h-3 rounded" style={{ background: d.label.includes("OK") ? "#16a34a" : d.label.includes("Non") ? "#ef4444" : d.label.includes("À") ? "#f59e0b" : "#9ca3af" }}></span>
                {d.label} — {d.value}
              </div>
            ))}
          </div>
          ...
          {barData.map((b) => (
            <div key={b.label}>
              <div className="flex items-center justify-between text-sm mb-1">
                <div>{b.label}</div>
                <div className="text-muted">Crit: {b.critical}</div>
              </div>
