...
router.post("/", async (req, res) => {
  try {
    ...
    const { resource, action, condition, only_roles, enabled } =
      req.body as any;
    if (!resource || !action || typeof condition === "undefined") {
      return res
        .status(400)
        .json({
          error: "Missing required fields: resource, action, condition",
        });
    }
    ...
    const payload: any = {
      ...
      only_roles: Array.isArray(only_roles)
        ? only_roles
        : only_roles
          ? String(only_roles)
              .split(",")
